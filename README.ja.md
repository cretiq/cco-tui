...
**このディレクトリで Claude Code セッションを開始すると、21.9K tokens が即座に context にロードされ、さらに 115.4K がオンデマンド MCP tools 用に deferred されます。** 200K の context window に対して、一文字も打つ前に 11% が消費済み — セッション中に Claude が MCP tools を呼び出すとさらに増えていきます。

Context Budget パネルの内訳：

- **Always Loaded** — CLAUDE.md、MEMORY.md index、skill descriptions、rules、system prompt と tools。毎回のリクエストで必ず context に入っています。
- **Deferred** — MCP tool schemas。Claude が ToolSearch 経由でオンデマンドにロードします。Claude がそのツールを必要とするまで context には入りません — ただし MCP サーバーが多いとすぐに膨らみます。

context が埋まるほど Claude の精度は落ちる — いわゆる **context rot** です。しかもこれらの数字はオフラインで計測できる部分だけ。セッション中、Claude Code はさらに以下を静かに追加します：

- **Rule re-injection** — すべての rule ファイルが tool call のたびに context に再注入される。~30回の tool call 後、これだけで context window の ~46% を消費し得る
- **File change diffs** — linter があなたが読んだファイルを変更した？差分全体が隠れた system-reminder として注入される
...
Claude はメモリ、スキル、MCP サーバー設定を、あなたの発言や操作から自動で作成します。古くなったもの、scope をまたいで重複したもの — どちらも毎セッションでロードされ続け、トークンを浪費します。閲覧、確認、削除。**Claude が何をロードするかは、あなたが決めます — Claude ではなく。**
...
> `npx @mcpware/claude-code-organizer` を実行して — Claude Code のすべてのリソースを管理するダッシュボードだよ。URLを教えて。

`http://localhost:3847` でダッシュボードが開きます。実際の `~/.claude/` ディレクトリを操作します。次回は Claude Code で `/cco` と入力するだけで再度開けます。
