...
**이 디렉토리에서 Claude Code 세션을 시작하면, 21.9K tokens가 즉시 context에 로드되고 추가로 115.4K가 on-demand MCP tools용으로 deferred됩니다.** 200K context window 기준으로 한 글자도 치기 전에 11%가 사라져요 — 세션 중 Claude가 MCP tools를 호출하면 더 늘어납니다.

Context Budget 패널의 구성:

- **Always Loaded** — CLAUDE.md, MEMORY.md index, skill descriptions, rules, system prompt와 tools. 매 request마다 반드시 context에 들어있어요.
- **Deferred** — MCP tool schemas. Claude가 ToolSearch를 통해 필요할 때 로드해요. Claude가 특정 tool을 필요로 하기 전엔 context에 안 들어있지만 — MCP server가 많으면 금방 쌓여요.

Context가 차면 찰수록 Claude 정확도가 떨어지는데, 이걸 **context rot**이라고 해요. 게다가 이 숫자들은 오프라인에서 측정 가능한 부분만이에요. 세션 중에 Claude Code는 조용히 더 추가합니다:

- **Rule re-injection** — 모든 rule 파일이 tool call마다 context에 재주입됩니다. ~30번 tool call 후, 이것만으로 context window의 ~46%를 차지할 수 있어요
- **File change diffs** — linter가 읽은 파일을 수정했다면? 전체 diff가 숨겨진 system-reminder로 주입돼요
...
Claude는 메모리, 스킬, MCP 서버 설정을 여러분이 말하거나 한 것에서 자동 생성해요. 어떤 건 오래돼서, 어떤 건 scope 간 중복돼서 — 그런데 전부 매 세션 로드되면서 토큰을 낭비합니다. 둘러보고, 읽고, 삭제. **Claude가 뭘 로드할지는 내가 정합니다 — Claude가 아니라.**
...
> `npx @mcpware/claude-code-organizer` 실행해줘 — Claude Code 리소스 전체를 관리하는 대시보드야. 준비되면 URL 알려줘.

`http://localhost:3847`에서 대시보드가 열립니다. 실제 `~/.claude/` 디렉토리를 직접 조작합니다. 다음에 다시 열고 싶으면 Claude Code에서 `/cco`만 입력하면 됩니다.
