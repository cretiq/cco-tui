...
**Nếu bạn mở session Claude Code trong thư mục này, 21.9K tokens được nạp ngay vào context, cộng thêm 115.4K deferred cho on-demand MCP tools.** Trên context window 200K, đó là 11% biến mất trước khi gõ một ký tự — và tăng dần khi Claude gọi MCP tools trong session.

Panel Context Budget phân tích như sau:

- **Always Loaded** — CLAUDE.md, MEMORY.md index, skill descriptions, rules, system prompt và tools. Những thứ này nằm trong context mỗi request.
- **Deferred** — MCP tool schemas mà Claude nạp on-demand qua ToolSearch. Chưa vào context cho đến khi Claude cần tool cụ thể — nhưng nếu bạn có nhiều MCP servers, chúng tích lũy rất nhanh.

Context càng đầy, Claude càng kém chính xác — gọi là **context rot**. Và những con số trên chỉ là phần đo offline được. Trong session, Claude Code âm thầm thêm nữa:
...
- **File change diffs** — linter sửa file bạn đọc? Toàn bộ diff tiêm vào dạng system-reminder ẩn
