...
**如果你喺呢個目錄底下開一個 Claude Code session，21.9K tokens 會即刻 load 入 context，另外仲有 115.4K deferred 俾 on-demand MCP tools。** 以 200K context window 計，你未打字已經用咗 11% — 而且 Claude 喺 session 入面用 MCP tools 嗰陣會繼續增加。

Context Budget 面板拆開嚟睇：

- **Always Loaded** — CLAUDE.md、MEMORY.md index、skill descriptions、rules、system prompt 同 tools。呢啲每一次 request 都喺你嘅 context 入面。
- **Deferred** — MCP tool schemas，Claude 透過 ToolSearch on-demand load。未用到嗰個 tool 之前唔會入 context — 但如果你有好多 MCP servers，加埋嚟都幾大。

Context 越滿 Claude 就越唔準 — 呢個叫 **context rot**。而且以上數字只係我哋可以離線量度嘅部分。Session 入面，Claude Code 仲會靜靜雞加多啲：

- **Rule re-injection** — 你所有 rule 檔案喺每次 tool call 之後都會重新注入。大約 30 次 tool call 之後，單係呢樣就食咗 ~46% context window
- **File change diffs** — 你讀過嘅檔案俾 linter 改咗？成個 diff 會以隱藏嘅 system-reminder 注入
...
Claude 會自動建立記憶、技能、MCP server 設定，源自你講過或做過嘅嘢。有啲過咗時，有啲喺唔同 scope 重複咗 — 但全部仲係每次 session 都 load，嘥晒 tokens。瀏覽、閱讀、刪除。**Claude load 啲乜，你話事 — 唔係 Claude。**
...
> 幫我跑 `npx @mcpware/claude-code-organizer` — 呢個係管理所有 Claude Code 資源嘅儀表板。跑起嚟之後話我知個 URL。

開 `http://localhost:3847`，直接操作你本機嘅 `~/.claude/` 目錄。下次想再開，喺 Claude Code 入面打 `/cco` 就得。
